...
import {
    ...
    LastValuesRequestElementModelV3,
    TimeValuePairModel
    ...
    private update() {
        ...
        this.destroy = this.dashboardService.initWidgetObservable.subscribe((event: string) => {
            if (event === 'reloadAll' || event === this.widget.id) {
                ...
                if (elements) {
                    const queries: LastValuesRequestElementModelV3[] = [];
                    elements.forEach((element: DeviceStatusElementModel) => {
                        if (element.exportId && element.exportValues) {
                            queries.push({exportId: element.exportId, columnName: element.exportValues.Name});
                        }
                    });

                    this.exportDataService.getLastValuesV3(queries).subscribe(res => {
                        this.items = [];
